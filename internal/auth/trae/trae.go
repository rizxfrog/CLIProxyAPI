...
// ConfigIdentity returns the machine/device id from the first configured
// trae-api-key entry that is not already an authenticated credential.
//
// The plain OAuth flow generates a random hex32 identity, but the checkin write
// endpoint rejects it with code 9074 while accepting the numeric AHA remote DID
// registered by the desktop client. Operators can pin that registered identity
// through trae-api-key device-id / machine-id so new logins reuse it.
func ConfigIdentity(cfg *config.Config) (machineID, deviceID string) {
	if cfg == nil {
		return "", ""
	}
	for _, entry := range cfg.TraeKey {
		...
		if strings.TrimSpace(entry.APIKey) != "" || strings.TrimSpace(entry.RefreshToken) != "" {
			// Entry already holds a credential; keep looking for an unclaimed one.
			continue
		}
...
func (c *Client) BuildLoginURL() (*LoginURL, error) {
	return c.BuildLoginURLWithIdentity("", "")
}

// BuildLoginURLWithIdentity behaves like BuildLoginURL but reuses caller-supplied
// machine/device ids when provided. Empty values fall back to random ones.
//
// The checkin write endpoint rejects the random hex32 identity generated by the
// plain OAuth flow with code 9074 while accepting the numeric AHA remote DID
// registered by the desktop client, so callers can pin a registered identity
// through trae-api-key device-id / machine-id.
func (c *Client) BuildLoginURLWithIdentity(machineID, deviceID string) (*LoginURL, error) {
	machineID = strings.TrimSpace(machineID)
	deviceID = strings.TrimSpace(deviceID)
	if machineID == "" {
		generated, errMachine := randomHex(16)
		if errMachine != nil {
	...
	if deviceID == "" {
		generated, errDevice := randomHex(16)
		if errDevice != nil {
